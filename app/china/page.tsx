    ...
    <main dir="rtl" className="min-h-screen overflow-hidden" style={{ "--china-primary": theme.primary, "--china-accent": theme.accent, "--china-background": theme.background, "--china-surface": theme.surface, "--china-ink": theme.ink } as CSSProperties}>
      ...
      <section aria-labelledby="china-opening-title" className="bg-[#7e1f27] px-5 pb-20 pt-14 text-[#fffaf1] sm:px-8 lg:pb-28 lg:pt-20">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
          ...
          <div className="rounded-[2rem] border border-[#f3d79d]/40 bg-[#211a18]/25 p-7"><span className="text-6xl">{chinaLandingConfig.flag}</span><p className="mt-6 text-sm font-black text-[#f3d79d]">تعلم فردي أونلاين</p><p className="mt-2 text-2xl font-black">القرآن أو العربية، وفق هدفك.</p><div className="mt-6 flex flex-wrap gap-2 text-sm font-bold text-[#f9e8d2]">{cities.map((city) => <span key={city} className="rounded-full border border-[#f3d79d]/50 px-3 py-2">{city}</span>)}</div></div>
        </div>
      </section>
      ...
      <section className="bg-[#fffaf1] px-5 py-16 sm:px-8"><div className="mx-auto max-w-6xl"><div className="grid gap-5 md:grid-cols-2"><article className="rounded-[2rem] bg-[#2f5d50] p-7 text-white"><p className="text-sm font-bold text-[#f3d79d]">تحفيظ القرآن الكريم</p><h2 className="mt-3 text-3xl font-black">روتين ثابت للحفظ والمراجعة</h2><p className="mt-4 leading-8 text-white/80">برنامج فردي للحفظ والمراجعة والقراءة، يبدأ من مستوى الطالب وهدفه التعليمي، مع تنظيم الحصص ضمن الباقة المختارة.</p></article><article className="rounded-[2rem] bg-[#211a18] p-7 text-white"><p className="text-sm font-bold text-[#e4bd6e]">تأسيس اللغة العربية</p><h2 className="mt-3 text-3xl font-black">أساس تدريجي للقراءة والفهم</h2><p className="mt-4 leading-8 text-white/75">برنامج تدريجي للطالب الذي يبدأ من الأساس أو يريد تحسين القراءة والفهم، مع متابعة تعليمية تناسب مستواه الحالي.</p></article></div></div></section>
      ...
      <section className="bg-[#c9953d] px-5 py-16 text-center text-[#211a18] sm:px-8"><div className="mx-auto max-w-3xl"><h2 className="text-4xl font-black">اختر برنامجك وتواصل معنا</h2><p className="mt-4 leading-8">اكتب مدينتك، والبرنامج الذي تريده، والوقت المناسب لك. سنستخدم هذه المعلومات لتنسيق الخطوة التالية باللغة العربية.</p><a href={trialUrl} target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-2 rounded-full bg-[#7e1f27] px-6 py-3 font-black text-white"><MessageCircle size={18} /> أريد حصة تجريبية</a></div></section>
