...
import { kuwaitLandingConfig, type KuwaitProgram } from "@/lib/kuwait-landing-config"
...
function Plans({ program, quranPrices, arabicPrices, contactUrl }: ProgramSelectorProps & { program: KuwaitProgram }) {
        ...
        {plans.map((plan, index) => (
          <article key={plan.sessions} className="flex flex-col rounded-2xl border border-kw-line bg-white p-6 shadow-[0_8px_24px_rgb(20_36_30/0.06)]">
            ...
            <div className="mt-6 border-y border-kw-line py-4">
              <strong className="text-4xl font-bold text-kw-green">{prices?.[index] ?? plan.price}</strong>
...
export default function KuwaitProgramSelector(props: ProgramSelectorProps) {
