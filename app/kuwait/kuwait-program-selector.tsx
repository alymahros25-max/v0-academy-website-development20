    ...
    <section id="programs" className="bg-kw-surface px-5 py-16 sm:px-8" aria-labelledby="programs-heading">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mx-auto mt-8 grid max-w-3xl gap-4 sm:grid-cols-2" role="tablist" aria-label="اختيار البرنامج">
          {(["quran", "arabic"] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={program === item} aria-controls={`${item}-plans`} onClick={() => setProgram(item)} className={`group relative min-h-28 rounded-2xl border-2 p-5 text-right shadow-[0_5px_0_#c8d1ca,0_14px_28px_rgb(15_93_60/0.09)] transition-all hover:-translate-y-0.5 hover:shadow-[0_7px_0_#c8d1ca,0_18px_30px_rgb(15_93_60/0.14)] active:translate-y-0.5 active:shadow-[0_3px_0_#c8d1ca,0_8px_18px_rgb(15_93_60/0.1)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-kw-gold/35 focus-visible:ring-offset-2 ${program === item ? "border-kw-green bg-kw-green text-white shadow-[0_5px_0_#0a3f28,0_14px_28px_rgb(15_93_60/0.2)]" : "border-kw-green/25 bg-white text-kw-ink hover:border-kw-green"}`}><span className="block text-lg font-bold">{item === "quran" ? "تحفيظ القرآن" : "تأسيس اللغة العربية"}</span><span className={`mt-2 block text-sm leading-6 ${program === item ? "text-white/80" : "text-kw-muted"}`}>{item === "quran" ? "حفظ وتسميع وتصحيح تلاوة ومراجعة ضمن حصة فردية." : "قراءة وكتابة ونطق وفهم وفق احتياج الطالب."}</span></button>)}
        </div>
