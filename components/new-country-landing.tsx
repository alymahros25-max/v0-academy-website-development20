...
import { getAreaLandingData, toAreaDisplayPlan } from "@/lib/country-content"
...
type Props = { config: NewCountryConfig; videos?: LandingVideo[]; teachers?: Teacher[] }
...
function TeacherTrust({ teachers }: { teachers: Teacher[] }) {
  ...
  const maleCount = activeTeachers.filter((teacher) => teacher.name.ar.includes("الشيخ") || teacher.name.ar.includes("أحمد") || teacher.name.ar.includes("عبد") || teacher.name.ar.includes("محمد")).length
  const femaleCount = activeTeachers.filter((teacher) => teacher.name.ar.includes("الأستاذة") || teacher.name.ar.includes("فاطمة") || teacher.name.ar.includes("مريم") || teacher.name.ar.includes("نورا")).length
  const maxExperience = activeTeachers.reduce((max, teacher) => Math.max(max, Number(teacher.experience) || 0), 0)
  return <section className="new-country-trust"><div className="new-country-narrow"><p className="new-country-kicker">فريق التعليم</p><h2>معلمون ومعلمات للطلاب والطالبات</h2><p>يضم الفريق معلمين للرجال والطلاب، ومعلمات للنساء والطالبات، من أصحاب الإجازات والخبرة المسجلة في لوحة التحكم. يتم اختيار المعلم أو المعلمة وفق البرنامج وعمر الطالب.</p><div className="new-country-trust-stats"><span><b>{maleCount || "—"}</b><small>معلمون</small></span><span><b>{femaleCount || "—"}</b><small>معلمات</small></span><span><b>{maxExperience || "—"}</b><small>سنوات خبرة قصوى</small></span></div></div></section>
}
...
function SharedClosing({ config, videos = [], teachers = [] }: Props) {
  return <><PackageIncludes /><NewCountryVideos videos={videos} /><TeacherTrust teachers={teachers} /><section className="new-country-section new-country-closing"><div className="new-country-narrow"><p className="new-country-kicker">خطوة عملية</p><h2>ابدأ بما يناسب أسبوعك الآن</h2><p>أرسل عمر الطالب ومستواه والبرنامج المطلوب، وسنوضح لك الخطوة التالية قبل التسجيل.</p><WhatsApp config={config} /><p className="new-country-independent-note">صفحة مستقلة لـ{config.name} · تعليم أونلاين فقط</p></div></section><CountryFooter config={config} /></>
}
...
function QatarLayout({ config, videos, teachers }: Props) {
...
function OmanLayout({ config, videos, teachers }: Props) {
  return <><section className="new-country-intro new-country-intro-oman"><div className="new-country-intro-copy"><p className="new-country-kicker">{config.flag} {config.eyebrow}</p><h1>{config.title}</h1><p className="new-country-lead">{config.description}</p><WhatsApp config={config} label="ابدأ بهدوء" /></div><div className="new-country-quiet-card"><span>قبل أن تبدأ</span><strong>مستوى الطالب</strong><small>الوقت المتاح · الهدف</small></div></section><section className="new-country-section new-country-paper"><div className="new-country-narrow"><p className="new-country-kicker">منهج متدرج</p><h2>الحفظ والمراجعة في ثلاث حركات</h2><Steps config={config} /></div></section><section className="new-country-section"><div className="new-country-narrow"><h2>اختيار الباقة</h2><PriceTable config={config} compact /></div></section><LocalSection config={config} /><FAQ config={config} /><section className="new-country-note"><b>يوجد باقات مخصصة</b><span>خصم 10٪ للأخوات والإحالة</span></section><SharedClosing config={config} videos={videos} teachers={teachers} /></>
}

function JordanLayout({ config, videos, teachers }: Props) {
...
function BahrainLayout({ config, videos, teachers }: Props) {
...
function LanguageLayout({ config, videos, teachers }: Props) {
  const isBelgium = config.variant === "belgium"
  return <><section className={`new-country-intro ${isBelgium ? "new-country-intro-belgium" : "new-country-intro-france"}`}><div className="new-country-language-card"><p className="new-country-kicker">{config.flag} {config.eyebrow}</p><h1>{config.title}</h1><p className="new-country-lead">{config.description}</p><div className="new-country-language-pills"><span>العربية</span><span>{isBelgium ? "المنطقة" : "اللغة العربية"}</span><span>{config.currency}</span></div><WhatsApp config={config} label="اسأل عن المسار" /></div></section><section className="new-country-section"><div className="new-country-narrow"><p className="new-country-kicker">بوابة الاختيار</p><h2>{isBelgium ? "البرنامج ثم المنطقة" : "البرنامج ثم اللغة"}</h2><Steps config={config} /></div></section><section className="new-country-section new-country-language-surface"><div className="new-country-narrow"><h2>باقات شهرية واضحة</h2><PriceTable config={config} /></div></section><LocalSection config={config} /><FAQ config={config} /><section className="new-country-note"><b>يوجد باقات مخصصة</b><span>خصم 10٪ للأخوات والإحالة</span></section><SharedClosing config={config} videos={videos} teachers={teachers}/></>
}

function SpainLayout({ config, videos, teachers }: Props) {
...
function NetherlandsLayout({ config, videos, teachers }: Props) {
...
export async function NewCountryLanding({ config }: Props) {
  ...
  const props = { config: dynamicConfig, videos, teachers }
