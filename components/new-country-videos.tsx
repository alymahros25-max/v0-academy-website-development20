...
export function NewCountryVideos({ videos }: Props) {
  ...
  return <section className="new-country-video-showcase" aria-labelledby="country-video-title">
    ...
    <div className="new-country-video-wall">{filtered.map((video, index) => <button type="button" className={`new-country-video-tile ${index === 0 ? 'is-featured' : ''}`} key={video.id} onClick={() => setActiveId(video.id)}><span className="new-country-video-thumb">{(video.thumbnail_url || video.poster) ? <Image src={video.thumbnail_url || video.poster || ""} alt={video.title_ar} width={640} height={360} sizes="(max-width: 768px) 100vw, 640px" className="object-cover" /> : <span className="block aspect-video bg-muted" aria-hidden="true" />}<span className="new-country-video-play"><Play size={20} fill="currentColor" /></span>{video.duration_seconds ? <small>{Math.floor(video.duration_seconds / 60)}:{String(video.duration_seconds % 60).padStart(2, '0')}</small> : null}</span><strong>{video.title_ar}</strong>{video.teacher_name_ar ? <em>{video.teacher_name_ar}</em> : null}</button>)}</div>
